...
from yolo_core.utils import load_yolo_weights, detect_image, image_preprocess, postprocess_boxes, nms, read_class_names
...
def get_mAP(Yolo, dataset, score_threshold=0.25, iou_threshold=0.50, TEST_INPUT_SIZE=TEST_INPUT_SIZE):
    ...
    # for index in range(dataset.num_samples):
    for index in tqdm.tqdm(range(dataset.num_samples)):
        ann_dataset = dataset.annotations[index]
...
if __name__ == '__main__':
    ...
    if YOLO_CUSTOM_WEIGHTS == False:
        ...
        # yolo.load_weights(f"./saved_model/{TRAIN_MODEL_NAME}") # use custom weights
