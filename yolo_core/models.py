...
def cspdarknet53(input_data):
    ...
    route = input_data
    route = DarknetConv2D(route, (1, 1, 64, 64), activate_type="mish")
    input_data = DarknetConv2D(input_data, (1, 1, 64, 64), activate_type="mish")
    for i in range(1):
        input_data = DarknetResidual(input_data,  64,  32, 64, activate_type="mish")
    input_data = DarknetConv2D(input_data, (1, 1, 64, 64), activate_type="mish")

    input_data = tf.concat([input_data, route], axis=-1)
    input_data = DarknetConv2D(input_data, (1, 1, 128, 64), activate_type="mish")
    input_data = DarknetConv2D(input_data, (3, 3, 64, 128), downsample=True, activate_type="mish")
    route = input_data
    route = DarknetConv2D(route, (1, 1, 128, 64), activate_type="mish")
    input_data = DarknetConv2D(input_data, (1, 1, 128, 64), activate_type="mish")
    for i in range(2):
        input_data = DarknetResidual(input_data, 64,  64, 64, activate_type="mish")
    input_data = DarknetConv2D(input_data, (1, 1, 64, 64), activate_type="mish")
    input_data = tf.concat([input_data, route], axis=-1)

    input_data = DarknetConv2D(input_data, (1, 1, 128, 128), activate_type="mish")
    input_data = DarknetConv2D(input_data, (3, 3, 128, 256), downsample=True, activate_type="mish")
    route = input_data
    route = DarknetConv2D(route, (1, 1, 256, 128), activate_type="mish")
    input_data = DarknetConv2D(input_data, (1, 1, 256, 128), activate_type="mish")
    for i in range(8):
        input_data = DarknetResidual(input_data, 128, 128, 128, activate_type="mish")
    input_data = DarknetConv2D(input_data, (1, 1, 128, 128), activate_type="mish")
    input_data = tf.concat([input_data, route], axis=-1)

    input_data = DarknetConv2D(input_data, (1, 1, 256, 256), activate_type="mish")
    route_1 = input_data
    input_data = DarknetConv2D(input_data, (3, 3, 256, 512), downsample=True, activate_type="mish")
    route = input_data
    route = DarknetConv2D(route, (1, 1, 512, 256), activate_type="mish")
    input_data = DarknetConv2D(input_data, (1, 1, 512, 256), activate_type="mish")
    for i in range(8):
        input_data = DarknetResidual(input_data, 256, 256, 256, activate_type="mish")
    input_data = DarknetConv2D(input_data, (1, 1, 256, 256), activate_type="mish")
    input_data = tf.concat([input_data, route], axis=-1)

    input_data = DarknetConv2D(input_data, (1, 1, 512, 512), activate_type="mish")
    route_2 = input_data
    input_data = DarknetConv2D(input_data, (3, 3, 512, 1024), downsample=True, activate_type="mish")
    route = input_data
    route = DarknetConv2D(route, (1, 1, 1024, 512), activate_type="mish")
    input_data = DarknetConv2D(input_data, (1, 1, 1024, 512), activate_type="mish")
    for i in range(4):
        input_data = DarknetResidual(input_data, 512, 512, 512, activate_type="mish")
    input_data = DarknetConv2D(input_data, (1, 1, 512, 512), activate_type="mish")
    input_data = tf.concat([input_data, route], axis=-1)

    input_data = DarknetConv2D(input_data, (1, 1, 1024, 1024), activate_type="mish")
    input_data = DarknetConv2D(input_data, (1, 1, 1024, 512))
    input_data = DarknetConv2D(input_data, (3, 3, 512, 1024))
    input_data = DarknetConv2D(input_data, (1, 1, 1024, 512))

    input_data = tf.concat([tf.nn.max_pool(input_data, ksize=13, padding='SAME', strides=1), tf.nn.max_pool(input_data, ksize=9, padding='SAME', strides=1)
                            , tf.nn.max_pool(input_data, ksize=5, padding='SAME', strides=1), input_data], axis=-1)
    input_data = DarknetConv2D(input_data, (1, 1, 2048, 512))
    input_data = DarknetConv2D(input_data, (3, 3, 512, 1024))
    input_data = DarknetConv2D(input_data, (1, 1, 1024, 512))
...
def YOLOv4(input_layer, NUM_CLASS):
    ...
    conv = DarknetConv2D(conv, (1, 1, 512, 256))
    conv = upsample(conv)
    route_2 = DarknetConv2D(route_2, (1, 1, 512, 256))
    ...
    conv = DarknetConv2D(conv, (1, 1, 256, 128))
    conv = upsample(conv)
    route_1 = DarknetConv2D(route_1, (1, 1, 256, 128))
    ...
    route_1 = conv
    conv = DarknetConv2D(conv, (3, 3, 128, 256))
    conv_sbbox = DarknetConv2D(conv, (1, 1, 256, 3 * (NUM_CLASS + 5)), activate=False, bn=False)

    conv = DarknetConv2D(route_1, (3, 3, 128, 256), downsample=True)
    conv = tf.concat([conv, route_2], axis=-1)
    ...
    route_2 = conv
    conv = DarknetConv2D(conv, (3, 3, 256, 512))
    conv_mbbox = DarknetConv2D(conv, (1, 1, 512, 3 * (NUM_CLASS + 5)), activate=False, bn=False)

    conv = DarknetConv2D(route_2, (3, 3, 256, 512), downsample=True)
    ...
    conv = DarknetConv2D(conv, (3, 3, 512, 1024))
    conv_lbbox = DarknetConv2D(conv, (1, 1, 1024, 3 * (NUM_CLASS + 5)), activate=False, bn=False)
